...
INPUT_DIR = r'D:\Astrophotography\2025-06-27\FLAT'
...
def collect_all_data(input_dir):
    ...
    for i in range(0, len(all_fits_files) - 1, 2):
        ...
        print(f"Processing pair: {processed_pairs_count:04d} {os.path.basename(img_path1)} and {os.path.basename(img_path2)}")
...
def analyze_signal_exposure_curve(all_collected_data, subframe_biases, plot_metadata, output_plot_file):
    ...
    # --- Automation for INITIAL_LINEAR_EXPOSURE_MAX_SECONDS ---
    num_points_for_initial_fit_heuristic = 5
    if len(all_exposure_times_sorted) >= num_points_for_initial_fit_heuristic:
        INITIAL_LINEAR_EXPOSURE_MAX_SECONDS_DYNAMIC = all_exposure_times_sorted[num_points_for_initial_fit_heuristic - 1]
    elif len(all_exposure_times_sorted) >= 2: # At least two points needed for a fit
        INITIAL_LINEAR_EXPOSURE_MAX_SECONDS_DYNAMIC = all_exposure_times_sorted[-1] * 0.5
        if INITIAL_LINEAR_EXPOSURE_MAX_SECONDS_DYNAMIC == 0:
            INITIAL_LINEAR_EXPOSURE_MAX_SECONDS_DYNAMIC = 0.1 # Small default if max_exp is 0
    else:
        print("Warning: Very few exposure points available for initial linear fit. Using default 1.0s for initial fit range.")
        INITIAL_LINEAR_EXPOSURE_MAX_SECONDS_DYNAMIC = 1.0


    # --- Dynamic Saturation Threshold Detection ---
    # Use the 99.5 percentile of raw signals as an estimate for saturation level
    dynamic_saturation_level = np.percentile(all_raw_signals_sorted, 99.5)
    # Define a threshold for fitting slightly below the detected saturation level
    ...
    mask_initial_fit = (all_exposure_times_sorted <= INITIAL_LINEAR_EXPOSURE_MAX_SECONDS_DYNAMIC) & \
    ...
    if np.sum(mask_initial_fit) >= 2:
        ...
        print(f"--- Light Curve Analysis (Initial Region <= {INITIAL_LINEAR_EXPOSURE_MAX_SECONDS_DYNAMIC:.2f}s) ---")
        ...
        print(f"Warning: Not enough unique light curve points for initial linear fit (using up to {INITIAL_LINEAR_EXPOSURE_MAX_SECONDS_DYNAMIC:.2f}s). Skipping analysis of this region.\n")
    ...
    if initial_fit_successful:
        ...
        jump_idx = -1
        # Start checking for jump only after the initial fit region, and above minimum exposure
        start_check_idx = np.where(all_exposure_times_sorted > INITIAL_LINEAR_EXPOSURE_MAX_SECONDS_DYNAMIC)[0]
        ...
        if jump_idx != -1:
            ...
            if len(valid_exposure_times_below_saturation) > 0:
                ...
                # Ensure a minimum range for the second fit
                if second_linear_max_exp <= second_linear_min_exp + 0.1:
                     # Fallback for second_linear_max_exp if the range is too small
                    potential_max_exp = np.max(all_exposure_times_sorted[all_raw_signals_sorted < dynamic_saturation_level]) * 0.95
                    if potential_max_exp > second_linear_min_exp:
                        second_linear_max_exp = potential_max_exp
                    else:
                        second_linear_max_exp = second_linear_min_exp + 0.1 # A very small range to at least attempt a fit
                    print("Warning: second_linear_max_exp adjusted for robust second fit range (too close to jump/saturation).\n")

            else:
                second_linear_max_exp = second_linear_min_exp + 0.1 # Minimal range if no points past jump
    ...
    if initial_fit_successful:
        ...
        fit_x1 = np.linspace(0, INITIAL_LINEAR_EXPOSURE_MAX_SECONDS_DYNAMIC, 100)
        plt.plot(fit_x1, initial_light_response_rate * fit_x1 + mean_subframe_bias, 'r--',
                 label=f'Linear Fit (0-{INITIAL_LINEAR_EXPOSURE_MAX_SECONDS_DYNAMIC:.2f}s): Y = {initial_light_response_rate:.2f}X + {mean_subframe_bias:.2f}')
    ...
    if initial_fit_successful:
        plt.axvline(INITIAL_LINEAR_EXPOSURE_MAX_SECONDS_DYNAMIC, color='darkorange', linestyle=':', label=f'1st Fit End ({INITIAL_LINEAR_EXPOSURE_MAX_SECONDS_DYNAMIC:.2f}s)')
...
def create_loglog_plots(all_collected_data, filename_sn, subframe_biases, subframe_biases_std, dynamic_saturation_adu, raw_signal_at_jump_point, plot_metadata):
    """
    Creates a log-log plot of bias-subtracted Signal vs. Noise and identifies key regions for gain.
    """
    ...
    # Setting reasonable limits for the plot
    # Automatically adjust xlim based on data range, but ensure it's not too small or too large
    min_signal_data = np.min(signals_for_plot) if len(signals_for_plot) > 0 else 1e-1
    max_signal_data = np.max(signals_for_plot) if len(signals_for_plot) > 0 else 1e5
    plt.xlim(max(1e-1, min_signal_data * 0.8), max(1e2, max_signal_data * 1.2))
    plt.ylim(1e1, 1e3) # Keep y-limits fixed as noise is typically in this range
    ...
    # --- Read Noise Calculation and Plotting ---
    ...
    if all_dark_noises:
        ...
        plt.text(plt.xlim()[0] * 1.5, read_noise_adu_mean * 1.1,
                 f'Read Noise: {read_noise_adu_mean:.2f} $\\pm$ {read_noise_adu_std:.2f} ADU',
    ...
    # --- Dynamic Photon Noise Fit Ranges (based on detected jump and saturation) ---
    read_noise_adu_mean_for_ranges = read_noise_adu_mean if not np.isnan(read_noise_adu_mean) else 10.0 # Fallback for read noise

    # Determine PHOTON_NOISE_SIGNAL_MIN_EFFECTIVE_REG1
    # Start above read noise, typically a few multiples of read noise or a fixed low value.
    dynamic_PN_MIN_REG1 = max(10, int(read_noise_adu_mean_for_ranges * 2))

    # Determine PHOTON_NOISE_SIGNAL_MAX_EFFECTIVE_REG1
    dynamic_PN_MAX_REG1 = 0
    if not np.isnan(effective_signal_at_jump_point_for_plot) and effective_signal_at_jump_point_for_plot > dynamic_PN_MIN_REG1 * 1.5:
        dynamic_PN_MAX_REG1 = effective_signal_at_jump_point_for_plot * 0.9 # Just before jump
    else: # No clear jump or jump is too early/low, use a wide range up to saturation (single gain region)
        dynamic_PN_MAX_REG1 = saturation_effective_adu * 0.9 if saturation_effective_adu > dynamic_PN_MIN_REG1 * 1.5 else np.max(signals_for_plot) * 0.9
        # Ensure there's a reasonable range if max_signal_data is small
        if dynamic_PN_MAX_REG1 <= dynamic_PN_MIN_REG1 + 100 and len(signals_for_plot) > 1:
            dynamic_PN_MAX_REG1 = np.percentile(signals_for_plot, 90) # Use 90th percentile if range is too small
        dynamic_PN_MAX_REG1 = max(dynamic_PN_MIN_REG1 + 10, dynamic_PN_MAX_REG1) # Ensure min < max

    # Determine PHOTON_NOISE_SIGNAL_MIN_EFFECTIVE_REG2 and PHOTON_NOISE_SIGNAL_MAX_EFFECTIVE_REG2
    fit_second_gain_region = False
    dynamic_PN_MIN_REG2 = 0
    dynamic_PN_MAX_REG2 = 0

    if not np.isnan(effective_signal_at_jump_point_for_plot) and effective_signal_at_jump_point_for_plot > 0:
        dynamic_PN_MIN_REG2 = effective_signal_at_jump_point_for_plot * 1.1 # Just after jump
        # Ensure there's enough room between jump and saturation for a second region
        if saturation_effective_adu > dynamic_PN_MIN_REG2 * 1.1:
            dynamic_PN_MAX_REG2 = saturation_effective_adu * 0.9
            # Ensure min < max for region 2 and it's not trivial
            if dynamic_PN_MAX_REG2 > dynamic_PN_MIN_REG2 + 10:
                fit_second_gain_region = True
            else:
                print("Info: Second gain region is too narrow between jump and saturation. Skipping second gain calculation.\n")
        else:
            print("Info: Saturation too close to jump point for a meaningful second gain region. Skipping second gain calculation.\n")
    else:
        print("Info: No clear jump detected in the light curve. Only one gain region will be analyzed.\n")

    # 1. Photon Noise Region 1 (Slope 1/2) - Gain 1 calculation ---
    photon_noise_mask_reg1 = (signals_for_plot >= dynamic_PN_MIN_REG1) & \
                             (signals_for_plot <= dynamic_PN_MAX_REG1)

    if np.sum(photon_noise_mask_reg1) >= 2: # Need at least 2 points for linregress
        signal_subset_reg1 = signals_for_plot[photon_noise_mask_reg1]
        noise_subset_reg1 = noises_for_plot[photon_noise_mask_reg1]
        ...
        if slope_ns2_reg1 > 0:
            ...
            gain_ADU_per_electron_1 = slope_ns2_reg1

            fit_signals_reg1 = np.linspace(dynamic_PN_MIN_REG1, dynamic_PN_MAX_REG1, 100)
            fitted_noise_photon_reg1 = np.sqrt(slope_ns2_reg1 * fit_signals_reg1 + intercept_ns2_reg1)
            fitted_noise_photon_reg1[fitted_noise_photon_reg1 <= 0] = np.nan # Avoid log(negative)
            plt.loglog(fit_signals_reg1, fitted_noise_photon_reg1, 'green', linestyle='-', linewidth=2,
                       label=f'Photon Noise Fit 1 (Log-Log Slope: {slope_loglog_reg1:.4f} $\\pm$ {stderr_loglog_reg1:.4f})')

            plt.text(np.mean(fit_signals_reg1), np.mean(fitted_noise_photon_reg1) * 0.5,
                     f'Gain 1: {gain_electrons_per_ADU_1:.3f} e-/ADU ({gain_ADU_per_electron_1:.4f} ADU/e-)',
                     color='green', horizontalalignment='center', verticalalignment='top', fontsize=10)
            print(f"--- Photon Noise Analysis Region 1 (Gain 1) ---")
            print(f"Fit Range (Effective Signal): {dynamic_PN_MIN_REG1:.0f} - {dynamic_PN_MAX_REG1:.0f} ADU")
            print(f"Gain 1 (from Noise^2 vs Signal): {gain_electrons_per_ADU_1:.3f} e-/ADU ({gain_ADU_per_electron_1:.4f} ADU/e-)")
        ...
        print(f"Warning: Not enough data points in Photon Noise REGION 1 (Effective Signal between {dynamic_PN_MIN_REG1} and {dynamic_PN_MAX_REG1} ADU). Adjust ranges or check data.\n")


    # 2. Photon Noise Region 2 (Slope 1/2) - Gain 2 calculation ---
    if fit_second_gain_region:
        photon_noise_mask_reg2 = (signals_for_plot >= dynamic_PN_MIN_REG2) & \
                                 (signals_for_plot <= dynamic_PN_MAX_REG2)

        if np.sum(photon_noise_mask_reg2) >= 2:
            signal_subset_reg2 = signals_for_plot[photon_noise_mask_reg2]
            noise_subset_reg2 = noises_for_plot[photon_noise_mask_reg2]
            ...
            if slope_ns2_reg2 > 0:
                ...
                gain_ADU_per_electron_2 = slope_ns2_reg2

                fit_signals_reg2 = np.linspace(dynamic_PN_MIN_REG2, dynamic_PN_MAX_REG2, 100)
                ...
                plt.loglog(fit_signals_reg2, fitted_noise_photon_reg2, 'forestgreen', linestyle='--', linewidth=2,
                           label=f'Photon Noise Fit 2 (Log-Log Slope: {slope_loglog_reg2:.4f} $\\pm$ {stderr_loglog_reg2:.4f})')

                plt.text(np.mean(fit_signals_reg2), np.mean(fitted_noise_photon_reg2) * 0.5,
                         f'Gain 2: {gain_electrons_per_ADU_2:.3f} e-/ADU ({gain_ADU_per_electron_2:.4f} ADU/e-)',
                ...
                print(f"Fit Range (Effective Signal): {dynamic_PN_MIN_REG2:.0f} - {dynamic_PN_MAX_REG2:.0f} ADU")
                print(f"Gain 2 (from Noise^2 vs Signal): {gain_electrons_per_ADU_2:.3f} e-/ADU ({gain_ADU_per_electron_2:.4f} ADU/e-)")
            ...
            print(f"Warning: Not enough data points in Photon Noise REGION 2 (Effective Signal between {dynamic_PN_MIN_REG2} and {dynamic_PN_MAX_REG2} ADU). Adjust ranges or check data.\n")


    # Jump Point and Saturation
    if not np.isnan(effective_signal_at_jump_point_for_plot) and effective_signal_at_jump_point_for_plot > 0:
    ...
    if saturation_effective_adu > 0:
        plt.axvline(saturation_effective_adu, color='purple', linestyle=':', linewidth=2, label='Approx. Saturation (Effective Signal)')
