...
INPUT_DIR = r'D:\Astrophotography\2025-06-11\FLAT' 
OUTPUT_CSV_FILE = 'signal_noise_data.csv'
OUTPUT_PLOT_FILE_SN_STANDARD = 'signal_noise_plot_standard.png'
...
READ_NOISE_SIGNAL_MAX_EFFECTIVE = 1            # Max bias-subtracted signal for estimating read noise
...
FPN_SIGNAL_MIN_EFFECTIVE_REG1 = 100000         # Min effective signal for FPN Fit 1 (less likely to be pure FPN in this region)
...
PHOTON_NOISE_SIGNAL_MIN_EFFECTIVE_REG2 = 20000 # Min effective signal for Photon Noise Fit 2 (after jump)
...
def calculate_subframe_biases(all_collected_data):
    ...
    Returns a list of 9 bias values.
    """
    ...
    subframe_bias_sums = [0.0] * 9
    subframe_bias_counts = [0] * 9

    for dark_entry in min_exp_dark_images:
        if len(dark_entry['raw_signal_subframes']) == 9:
            for j in range(9):
                subframe_bias_sums[j] += dark_entry['raw_signal_subframes'][j]
                subframe_bias_counts[j] += 1
        else:
            print(f"Warning: Unexpected number of subframes ({len(dark_entry['raw_signal_subframes'])}) in a dark image entry. Skipping.")

    subframe_biases = [s / c if c > 0 else 0.0 for s, c in zip(subframe_bias_sums, subframe_bias_counts)]
    
    print(f"\n--- Subframe Bias Calculation (from DARK images at {min_dark_exp_time}s) ---")
    for i, bias_val in enumerate(subframe_biases):
        print(f"  Subframe {i}: {bias_val:.2f} ADU Bias")
    print(f"--------------------------------------------------")

    return subframe_biases
...
def analyze_signal_exposure_curve(all_collected_data, subframe_biases, plot_metadata, output_plot_file):
    ...
    if np.sum(mask_initial_fit) >= 2:
        ...
        slope_initial_fit, intercept_initial_fit, r_value_initial_fit, _, _ = linregress(exposure_times_initial_fit, raw_signals_initial_fit)
        
        initial_light_response_rate = slope_initial_fit
        initial_fit_successful = True

        print(f"\n--- Light Curve Analysis (Initial Region <= {INITIAL_LINEAR_EXPOSURE_MAX_SECONDS}s) ---")
        print(f"Fitted linear model (Initial Region): Signal = {initial_light_response_rate:.2f} * ExposureTime + {intercept_initial_fit:.2f}")
        print(f"  (Note: Plot will show fit passing through {mean_subframe_bias:.2f} ADU at 0s, using calculated subframe biases)")
        print(f"Estimated Light Current Rate (Initial Region): {initial_light_response_rate:.2f} ADU/s")
        print(f"R-squared for fit: {r_value_initial_fit**2:.4f}")
        print(f"--------------------------------------------------")
    else:
    ...
    if initial_fit_successful: 
        ...
        extrapolated_initial_signals = initial_light_response_rate * all_exposure_times_sorted + intercept_initial_fit
        ...
        if jump_idx != -1:
            ...
            if len(valid_exposure_times_below_saturation) > 0:
                ...
                second_linear_max_exp = np.max(all_exposure_times_sorted) 
            ...
            if np.sum(mask_second_fit) >= 2:
                ...
                second_intercept_fit = intercept_second_fit # This is the intercept of the second fitted line
                second_fit_successful = True

                print(f"\n--- Second Linear Region Fit (LIGHT Images, Detected between {second_linear_min_exp:.2f}s and {second_linear_max_exp:.2f}s) ---")
                print(f"Fitted linear model (Second Region): Signal = {second_light_response_rate:.2f} * ExposureTime + {second_intercept_fit:.2f}")
...
def create_loglog_plots(all_collected_data, filename_sn, subframe_biases, dynamic_saturation_adu, raw_signal_at_jump_point, plot_metadata):
    """
    ...
    # This point comes from the overall light curve analysis, which used mean_subframe_bias for its plot
    mean_subframe_bias = np.mean(subframe_biases) if len(subframe_biases) > 0 else 0.0
    ...
    if np.sum(photon_noise_mask_reg1) > 1:
        ...
        slope_ns2_reg1, intercept_ns2_reg1, r_value_reg1, _, _ = linregress(signal_subset_reg1, noise_squared_subset_reg1)
        ...
        if slope_ns2_reg1 > 0:
            ...
            plt.text(gain_x_intercept_1 * 1.05, plt.ylim()[1] * 0.7, 
                     f'Gain 1: {gain_electrons_per_ADU_1:.3f} e-/ADU', 
                     color='green', rotation=90, verticalalignment='top', fontsize=10)
        else:
            print("Warning: Photon noise fit REGION 1 resulted in non-positive slope. Cannot calculate gain.")
    else:
    ...
    if np.sum(photon_noise_mask_reg2) > 1:
        ...
        slope_ns2_reg2, intercept_ns2_reg2, r_value_reg2, _, _ = linregress(signal_subset_reg2, noise_squared_subset_reg2)
        ...
        if slope_ns2_reg2 > 0:
            ...
            plt.text(gain_x_intercept_2 * 1.05, plt.ylim()[1] * 0.7, 
                     f'Gain 2: {gain_electrons_per_ADU_2:.3f} e-/ADU', 
                     color='forestgreen', rotation=90, verticalalignment='top', fontsize=10)
        else:
            print("Warning: Photon noise fit REGION 2 resulted in non-positive slope. Cannot calculate gain.")
    else:
    ...
    if np.sum(fpn_mask_reg1) > 1:
        ...
        slope_fpn_reg1, intercept_fpn_reg1, r_value_reg1, _, _ = linregress(fpn_signal_subset_log_reg1, fpn_noise_subset_log_reg1)
        ...
        if slope_fpn_reg1 != 0 and not np.isinf(slope_fpn_reg1) and not np.isnan(slope_fpn_reg1):
            ...
            if 100 < fpn_x_intercept_adu_reg1 < 100000: 
                ...
                plt.text(fpn_x_intercept_adu_reg1 * 1.02, plt.ylim()[0] * 1.5, 
                         f'FPN 1: {fpn_x_intercept_adu_reg1:.0f} ADU\n(slope {slope_fpn_reg1:.2f})', 
                         color='blue', rotation=90, verticalalignment='bottom', fontsize=10)
            else:
                print(f"Warning: Calculated FPN X-intercept REGION 1 {fpn_x_intercept_adu_reg1:.0f} ADU is outside typical plot range, not annotating.")
        else:
            print("Warning: FPN slope REGION 1 is zero or invalid. Cannot calculate or plot x-intercept as described.")
    else:
    ...
    if np.sum(fpn_mask_reg2) > 1:
        fpn_signal_subset_log_reg2 = np.log10(effective_signals_all[fpn_mask_reg2])
        fpn_noise_subset_log_reg2 = np.log10(noises_all[fpn_mask_reg2])

        slope_fpn_reg2, intercept_fpn_reg2, r_value_reg2, _, _ = linregress(fpn_signal_subset_log_reg2, fpn_noise_subset_log_reg2)
        ...
        if slope_fpn_reg2 != 0 and not np.isinf(slope_fpn_reg2) and not np.isnan(slope_fpn_reg2):
            ...
            if 100 < fpn_x_intercept_adu_reg2 < 100000: 
                ...
                plt.text(fpn_x_intercept_adu_reg2 * 1.02, plt.ylim()[0] * 1.5, 
                         f'FPN 2: {fpn_x_intercept_adu_reg2:.0f} ADU\n(slope {slope_fpn_reg2:.2f})', 
                         color='steelblue', rotation=90, verticalalignment='bottom', fontsize=10)
            else:
                 print(f"Warning: Calculated FPN X-intercept REGION 2 {fpn_x_intercept_adu_reg2:.0f} ADU is outside typical plot range, not annotating.")
        else:
            print("Warning: FPN slope REGION 2 is zero or invalid. Cannot calculate or plot x-intercept as described.")
    else:
...
if __name__ == "__main__":
    ...
    if all_collected_data_points:
        ...
        subframe_biases_list = calculate_subframe_biases(all_collected_data_points)
        ...
        print("No valid data collected. Please check the input directory and FITS file formats and ensure dark frames are present.")